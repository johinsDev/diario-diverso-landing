...
import { loadCategoriesPosts, loadLastPosts, loadPostBySlug, loadSimilarPosts } from "@/sanity/loader/loadQuery";
...
export default async function BlogDetailPage({ params }: Props) {
  ...
  const otherPosts = [...similarPost.data, ...lastPost]
    .filter((post, index, self) => self.findIndex((p) => p._id === post._id) === index)
    .slice(0, 3);
  ...
  return <main className="flex-1 flex flex-col gap-12 container py-4 md:py-10">
    ...
    {!!otherPosts.length && <section className="w-full justify-center text-center mt-8">
      {/* more blogs */}
